...
void sort_pairs(void)
{
    // Implement the bubble sort algorithm in decreasing order
    for (int i = 0; i < pair_count - 1; i++)
    {
        for (int j = 0; j < pair_count - i - 1; j++)
        {
            // The < sign was used instead of the > sign to sort in decreasing order
...
bool is_cycle(int winner, int loser)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        if (winner == pairs[i].loser)
        {
            ...
            {
                if (is_cycle(pairs[i].winner, loser))
                {
                    return true;
                }
            }
...
void print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count ; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            if (locked[i][j] && initialized == false)
            {
                winner = i;
                initialized = true;
            }
